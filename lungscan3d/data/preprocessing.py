...
def world_to_voxel_zyx(
    ...
    spacing_xyz = np.array(
        [spacing_zyx[2], spacing_zyx[1], spacing_zyx[0]], dtype=np.float32
    )
    voxel_xyz = (
...
def preprocess_candidate_csv(
    ...
    """
    LOGGER.info(
        "Preprocessing candidates: raw_dir=%s, metadata_csv=%s", raw_dir, metadata_csv
    )
    ...
    for row in rows.itertuples(index=False):
        seriesuid = str(getattr(row, "seriesuid"))
        if seriesuid not in mhd_index:
            LOGGER.warning(
                "Skipping candidate because CT file is missing: seriesuid=%s", seriesuid
            )
            continue
        ...
        coord_xyz = (
            float(getattr(row, "coordX")),
            float(getattr(row, "coordY")),
            float(getattr(row, "coordZ")),
        )
        ...
        patch = crop_or_pad_patch(
            ct_array, center_zyx=center_zyx, patch_size=patch_size
        )
...
def _extract_label(row: Any, task: str) -> float:
    ...
    if hasattr(row, "label"):
        return float(getattr(row, "label"))
    if task == "nodule_vs_non_nodule" and hasattr(row, "class"):
        return float(getattr(row, "class"))
    raise ValueError(
        "Could not extract a LUNA16 binary candidate label from metadata row"
    )
